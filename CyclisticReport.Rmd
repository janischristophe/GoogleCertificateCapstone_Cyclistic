...
+===============+=============================================+
| Define and    | -   Create scope of work documentation      |
| Discuss the   | -   Identify problem statement and          |
| Project       |     stakeholders                            |
|               | -   Define goals                            |
|               | -   Establish metrics of success and        |
|               |     expectations                            |
+---------------+---------------------------------------------+
| Prepare the   | -   Identify source of data, collect, and   |
| Data for      |     store                                   |
| Exploration   | -   Determine data reliability, origin,     |
|               |     comprehensiveness, current relevance,   |
|               |     credibility, and licensing              |
|               | -   Identify data organization; sort and    |
|               |     filter                                  |
+---------------+---------------------------------------------+
| Process the   | -   Determine the tools that will be used   |
| Data for      |     for the analysis/why                    |
| Analysis      | -   Check for data errors/ “dirty” data     |
|               | -   Transform data to correct data types    |
|               | -   Ensure that data being used is clean    |
|               | -   Document the cleaning process           |
+---------------+---------------------------------------------+
| Conduct a     | -   Organize, and format data properly      |
| Descriptive   | -   Identify trends in the data with        |
| Analysis      |     calculations, data aggregation, and     |
|               |     visualization                           |
|               | -   Provide a detailed analytical summary   |
|               |     of the clean, unbiased and              |
|               |     representative data                     |
+---------------+---------------------------------------------+
| Share         | -   What story is the analysis telling and  |
| Findings with |     how does it relate to the business      |
| Stakeholders  |     problem?                                |
|               | -   Determine way best communicate findings |
|               |     to the audience                         |
|               | -   Create an effective and accessible      |
|               |     presentation that helps solve the       |
|               |     business problem discussed beforehand   |
+---------------+---------------------------------------------+
| Act on Key    | -   Explore how additional data parameters  |
| Findings      |     would expand upon the analysis and      |
|               |     insights                                |
|               | -   Share recommendations based on          |
|               |     analytical findings, with shareholders  |
|               |     to to help the marketing team in their  |
|               |     project.                                |
+---------------+---------------------------------------------+
...
-   Flexible pricing plans:
    ...
    -   Full-day pass
    -   Annual membership
-   Options for types of bikes used (inclusive usage for those who
    cannot use a standard two-wheeled bike–8% of users use assistive
    options):
    -   Standard bicycle
    -   Reclining bikes
    -   Hand tricycles
...
+----------------+----------------------+----------------------+
| Stakeholders   | Establish Metrics of | Project Goals        |
|                | Success and          |                      |
|                | Expectations         |                      |
+================+======================+======================+
| Marketing      | Discover differences | Provide data-driven  |
| Analytics Team | in usage between the | results to inform    |
|                | two bike user types  | Cyclistic’s decision |
|                | and their            | making               |
|                | motivations          |                      |
+----------------+----------------------+----------------------+
| Lily Moreno,   | Gather data-driven   | Create a marketing   |
| Director of    | evidence to allow    | strategy to convert  |
| Marketing      | the marketing team   | as many casual       |
|                | to proceed with new  | riders to annual     |
|                | strategies           | members as possible  |
+----------------+----------------------+----------------------+
| Cyclistic      | Observe compelling   | Implement            |
| Executive Team | and relevant         | initiatives based on |
|                | insights to allow    | the data to advance  |
|                | advancement in       | Cylistic’s business  |
|                | marketing decisions  | growth               |
+----------------+----------------------+----------------------+
...
-   Data Analytical Standards
    -   🗷Reliability:
        ...
        -   there are several instances of negative ride time duration
            (an impossibility) that needs to be further investigated
        -   missing data points for some records, but they will be
            unused attributes at this point in the analysis

    -   🗹Originality: primary-source repository, with no outside
        influence

    -   🗹Comprehensiveness: allows for observations and comparisons
        between casual and member usership

    -   🗹Current: within time-frame parameters that we were asked to
        review
...
-   Data Ethics
    -   Security: data-sets are managed through a reputable and
        authorized cloud repository which provides safety and security
    -   Privacy: all data points have been anonymized to protect
...
-   Data Organization
    -   We have randomized data points organized by the first quarters
        (Jan. 1-Mar. 31) of 2019 and 2020. I will not be filtering out
        any data points at this moment of the data analysis process.
...
12. Observe the structure of both tables for any incongruities
    ...
    ![](images/structure_q1_2019.png){width="677"}![](images/structure_q1_2020.png)
...
20. Remove “bad” data from *all_trips* creating a second
    ...
    -   The *all_trips* data frame includes bikes that were taken out of
        docking stations for quality assurance purposes, but were
        counted as actual trips. These data points resulted in negative
        ride lengths that were observed in Excel. Now that we know why
        these ride lengths are present, we can remove them since they do
        not actually contribute to the analysis.
    -   There are a lot of rows that simply have “*nulls*” across the
        board with no other supporting data, however they do contribute
        to the number of rows in the column, so it is beneficial to
...
![The numbers were not necessarily expected here. Members tend to have
way more rides than casual rides, which makes complete sense. However,
the average ride lengths per group did come at a shock. Casual riders,
on average, use the bike over **seven times** longer than members use
the bike per trip.](images/pivot_table_2020.png)

2.  Collect the average, median, maximum, and minimum of *all_trips_v2*
    data frame

![Ride duration average is 0:13:51, the median is 0:08:59, the maximum
is 23:57:54, and the minimum is 1
second.](images/ride_length_mean_median_max_min.png){width="254"}

3.  Aggregate comparisons of ride duration averages, medians, maximums
    and minimums between user type classes

![In every category, casual riders tend to use the bikes for longer
periods of time than people who have the annual
membership.](images/usertypeVSride_length_mean_median_max_min.png){width="503"}

4.  Find the busiest days of the week

![As we can see, workdays (Monday-Friday) generate the most usage for
annual members and casual riders tend to use the bikes more on Saturday
and Sunday.](images/usertypeVSride_length_by%20day.png)
...
-   Of the 787,899 total rides completed during the two first quarters,
    720,249 (91.4%) of the users are annual members and 67,650 (8.6%)
    are casual riders.
-   For members, weekdays tend to get the most action. Monday thru
    Friday have high usage while Saturday and Sunday have about half the
    traffic.
-   Casual riders tend to use the network on Saturday and Sunday (the
    weekend taking up almost half of the data points). Monday thru
    Friday cover a little over half of the data points and are fairly
    equivalent in usage.
-   Peak usage times for members are around commuting times–5am to 9am
    and 3pm to 7pm–with peaks around 8am and 5pm.
-   Usage times for casual users are fairly steady throughout the day
    with 11am to 7pm being the busiest times for this group.
-   Although annual members are the majority of users–dominating the
    ...
    because members are going to use the bikes more than the casual
    users, but the ride length averages did come as a surprise. Casual
    riders use the bikes almost 8 times longer than members
...
-   Bike twice as much on the weekend (Saturday and Sunday) when
